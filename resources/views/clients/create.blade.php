        ...
        <div class="relative bg-[#1a1c23]/40 backdrop-blur-3xl border border-white/10 rounded-[2rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] flex flex-col max-h-[85vh]">
            ...
            <div class="flex items-center justify-between p-8 pb-4 shrink-0">
                ...
                <div class="text-center flex-1">
                    <h2 class="text-2xl font-black text-white tracking-tight uppercase">{{ isset($client) ? 'Edit Client' : 'New Client Profile' }}</h2>
            ...
            <form action="{{ isset($client) ? route('clients.update', $client) : route('clients.store') }}" method="POST" class="flex flex-col flex-1 min-h-0">
                @csrf
                @if(isset($client))
                    @method('PUT')
                @endif
