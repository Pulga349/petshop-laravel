        ...
        <div class="relative bg-[#1a1c23]/40 backdrop-blur-3xl border border-white/10 rounded-[2rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.6)] flex flex-col max-h-[85vh]">
            ...
            <div class="flex items-center justify-between p-8 pb-4 shrink-0">
                ...
                <div class="text-center flex-1">
                    <h2 class="text-2xl font-black text-white tracking-tight uppercase">Editar Detalles de Producto</h2>
            ...
            <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" class="flex flex-col flex-1 min-h-0">
                @csrf
                @method('PUT')
                ...
                <div class="flex-1 overflow-y-auto px-8">
                    ...
                    <div class="flex items-center gap-6 mb-6 group" x-data="{ photoPreview: '{{ $product->image ? asset('storage/'.$product->image) : null }}' }">
                        <div class="relative">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="number" name="purchase_price" step="0.01" value="{{ old('purchase_price', $product->purchase_price) }}" required
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="number" name="initial_stock" value="{{ old('initial_stock', $product->initial_stock ?? 0) }}" min="0"
                                   class="w-full bg-white/5 border border-white/5 rounded-xl px-4 py-2.5 text-sm text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/30 transition-all"
                                   placeholder="0">
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5 mt-4">
                        ...
                        <textarea name="description" rows="2"
                                  ...
                                  placeholder="Describa las características y beneficios...">{{ old('description', $product->description) }}</textarea>
                    </div>
                </div>
            ...
            <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="space-y-1.5 mt-4">
                    <label class="text-[9px] font-black text-gray-500 uppercase tracking-[0.2em] ml-1">Descripción</label>
                    <textarea name="description" rows="2"
                              class="w-full bg-white/5 border border-white/5 rounded-xl px-4 py-2 text-sm text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/30 transition-all placeholder:text-gray-700 resize-none">{{ old('description', $product->description) }}</textarea>
                </div>

                <!-- Footer Buttons -->
                <div class="flex items-center justify-between gap-4 mt-8 pt-6 border-t border-white/5">
                    <a href="{{ route('products.index') }}" 
                       class="px-6 py-3 bg-white/5 hover:bg-white/10 text-white text-xs font-black rounded-xl transition-all border border-white/5 uppercase tracking-widest">
                        Cancelar
                    </a>
